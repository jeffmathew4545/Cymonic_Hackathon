def decide(slot: dict) -> dict:
    ...
    fill_rate_component = (1.0 - fill_rate) * 0.6
    lead_time_component = (1.0 - min(lead_time_hours / 48.0, 1.0)) * 0.4
    risk_score = round(fill_rate_component + lead_time_component, 2)

    # 2. Compute Margin Guardrail
    max_safe_discount = max(0, int(margin_pct) - 10)
    ...
    if risk_score < 0.40:
        ...
        discount_pct = min(15, max_safe_discount)
        ...
        action = "notify_and_discount"
        discount_pct = min(25, max_safe_discount)
